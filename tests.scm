...
; expect (1 + 1)
(quote '''(1 2 3))
; expect (quote (quote (quote (1 2 3))))
(begin pi (* num 2) (begin (+ 1 2) (- 1 10)))
; expect -9
(+ 1 2 (begin (+ 1 2) 5))
; expect 8
(define g (lambda (let x y) (* let x y)))
; expect g
(g 1 2)
; expect Error
(g 1 2 3)
; expect 6
((lambda (x y) (begin (+ x y) (- x y))) 1 2)
; expect -1
(define (tester x)
  (define (tester2 y)
    (cons (+ x y) (cons (- x y) (cons x (cons y nil))))
  )
  tester2
)
; expect tester
...
(let ((a 1)
  ...
  (f))
; expect 2
(begin (define x (mu (a) (+ a b c) )) (define b 3) (define c 4) (x 2))
; expect 9
